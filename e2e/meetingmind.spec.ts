...
test.describe("MeetingMind Signal Room E2E Flow", () => {
  test("loads landing page and renders Payment Bug Triage fixture timeline & intelligence cards", async ({ page }) => {
    ...
    await expect(page.locator("h2").first()).toContainText("Payment Bug Triage");
    ...
    await expect(page.locator("span", { hasText: "DECISION" }).first()).toBeVisible();
    await expect(page.locator("span", { hasText: "ACTION ITEM" }).first()).toBeVisible();
    await expect(page.locator("span", { hasText: "RISK" }).first()).toBeVisible();
    await expect(page.locator("span", { hasText: "DISAGREEMENT" }).first()).toBeVisible();

    // Click an Intelligence Card and verify source quote selection
    await page.click("text=Run database index migration script");
    const sourceQuote = page.locator("text=Marcus Vance will run the database index migration script").first();
